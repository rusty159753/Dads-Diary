...
// Use the inferred return type to avoid generic type conflicts
...
async function handleCheckoutCompleted(supabase: ServiceClient, session: Stripe.Checkout.Session) {
...
async function handleSubscriptionUpdated(supabase: ServiceClient, stripeSubscription: Stripe.Subscription) {
  ...
  switch (stripeSubscription.status) {
    ...
    case 'trialing': status = 'active'; break
...
async function handleSubscriptionDeleted(supabase: ServiceClient, stripeSubscription: Stripe.Subscription) {
  const customerId = stripeSubscription.customer as string
  const { data: sub } = await supabase
    .from('subscriptions')
    .select('user_id')
